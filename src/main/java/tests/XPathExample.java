...
public class XPathExample extends BaseTest{
	...
	public void xpathExample2() {
		...
		// text --> citeste textul dintre tag-urile HTML
		...
		//direct child --> /
		WebElement oderTableHeader = driver.findElement(By.xpath("//th[contains(@class, 'woocommerce-orders-table__header')]/span[text()='Order']"));
		jse.executeScript("arguments[0].setAttribute('style', 'border:4px solid red')", oderTableHeader);
		
		//index based
		WebElement totalTableHeader = driver.findElement(By.xpath("(//th[contains(@class, 'woocommerce-orders-table__header')]/span)[1]"));
		jse.executeScript("arguments[0].setAttribute('style', 'border:4px solid orange')", totalTableHeader);
		
		WebElement orderTableHeader = driver.findElement(By.xpath("//td[contains(@class, 'woocommerce-orders-table__cell')][@data-title='Order']/a[not(contains(text(), '#172' ))]"));
		orderTableHeader.click();
